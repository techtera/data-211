"""
VGGT Unified — Standalone Inference Script (no external file dependencies).

Usage:
    # Cascade mode (obj + edge with ROI masking)
    python inference_standalone_unified.py --image path/to/image.png --checkpoint unified.pt --task cascade

    # Object mask only
    python inference_standalone_unified.py --image path/to/image.png --checkpoint unified.pt --task obj

    # Edge mask only
    python inference_standalone_unified.py --image path/to/image.png --checkpoint unified.pt --task edge

    # Both outputs
    python inference_standalone_unified.py --image path/to/image.png --checkpoint unified.pt --task both

    # Batch processing
    python inference_standalone_unified.py --image_dir path/to/images/ --checkpoint unified.pt --task cascade --output_dir results/
"""
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None) -> Tensor:
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            x = drop_add_residual_stochastic_depth(x, pos=pos, residual_func=attn_residual_func,
...
# ============================================================
# Object Mask Decoder: SegFormer Components
# ============================================================
...
class SegFormerDecoder(nn.Module):
    def __init__(self, in_channels=[256, 512, 1024, 1024], embedding_dim=256, num_classes=2):
        super().__init__()
...
class ObjMaskDecoder(nn.Module):
    ...
    def _forward_impl(self, aggregated_tokens_list, images, patch_start_idx, frames_start_idx=None, frames_end_idx=None):
...
# ============================================================
# Edge Mask Decoder: UNet++ Components
# ============================================================
...
class VGGTUnified(nn.Module):
    def __init__(self):
        ...
        self.obj_decoder = ObjMaskDecoder(dim_in=2048, output_dim=2)
    ...
    def forward(self, images: torch.Tensor, task: str = "both"):
        ...
        if task in ("obj", "both"):
            obj_logits = self.obj_decoder(aggregated_tokens_list, images=images, patch_start_idx=patch_start_idx)
            if obj_logits.dim() == 4:
...
def load_model(checkpoint_path, device):
    ...
    fp16 = checkpoint.get("config", {}).get("fp16", False)
    if fp16:
...
@torch.no_grad()
def predict(model, image_tensor, device, task="cascade"):
    image_tensor = image_tensor.to(device, non_blocking=True)
    with torch.amp.autocast(device_type="cuda", enabled=(device.type == "cuda")):
        results = model(image_tensor, task=task)
    return results
...
def save_output(results, image_path, output_dir, task):
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    stem = Path(image_path).stem

    if "obj_mask" in results:
        obj_logits = results["obj_mask"]
        obj_probs = torch.softmax(obj_logits, dim=2)
        obj_mask = obj_probs.argmax(dim=2).squeeze().cpu().numpy()
        obj_mask = (obj_mask * 255).astype(np.uint8)
        obj_mask_resized = cv2.resize(obj_mask, (1280, 720), interpolation=cv2.INTER_NEAREST)
        obj_path = output_dir / f"{stem}_obj_mask.png"
        Image.fromarray(obj_mask_resized, mode="L").save(obj_path)
        print(f"  Saved: {obj_path}")

    if "edge_mask" in results:
        edge_probs = results["edge_mask"].squeeze().cpu().numpy()
        edge_binary = (edge_probs >= 0.5).astype(np.uint8)
        edge_resized = cv2.resize(edge_binary, (1280, 720), interpolation=cv2.INTER_NEAREST)

        if task == "cascade":
            edge_skeleton = skeletonize(edge_resized > 0).astype(np.uint8) * 255
            edge_path = output_dir / f"{stem}_edge_roi.png"
            Image.fromarray(edge_skeleton, mode="L").save(edge_path)
            print(f"  Saved: {edge_path}")
        else:
            edge_resized = (edge_resized * 255).astype(np.uint8)
            edge_path = output_dir / f"{stem}_edge_mask.png"
            Image.fromarray(edge_resized, mode="L").save(edge_path)
            print(f"  Saved: {edge_path}")
...
def main():
    ...
    for img_path in image_paths:
        ...
        results = predict(model, image_tensor, device, args.task)
        ...
        save_output(results, img_path, output_dir, args.task)
